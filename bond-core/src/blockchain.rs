...
use crate::mining::{Miner, DifficultyAdjuster, MiningResult};
...
impl Blockchain {
    ...
    pub fn get_block_by_height(&self, height: u64) -> Option<&Block> {
        if (height as usize) < self.blocks.len() {
    ...
    pub fn calculate_block_reward(&self, _height: u64) -> u64 {
    ...
    pub fn calculate_total_fees(&self, transactions: &[Transaction]) -> Result<u64> {
        ...
        for tx in transactions {
            ...
            total_fees = total_fees.checked_add(fee)
                .ok_or(BlockchainError::InvalidTransaction("Fee overflow".to_string()))?;
        }
    ...
    pub fn get_next_difficulty(&self) -> u32 {
        ...
        adjuster.calculate_new_difficulty(current_difficulty, &self.blocks)
    ...
    pub fn create_transaction(
        ...
        let utxos = self.utxo_set.find_utxos_for_amount(from_script, total_needed)?;
        ...
        let inputs: Vec<_> = utxos.iter()
            .map(|utxo| crate::transaction::TxInput::new(
                utxo.outpoint(),
        ...
        if change > 0 {
            outputs.push(crate::transaction::TxOutput::new(change, from_script.to_vec()));
        }
    ...
    pub fn stats(&self) -> BlockchainStats {
        let total_supply = self.blocks.iter()
